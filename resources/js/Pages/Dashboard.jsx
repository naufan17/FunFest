            ...
            {isAdmin && (
                <div className="space-y-6 md:space-y-10">
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                        <div>
                            <h2 className="text-xs md:text-sm font-black uppercase tracking-[0.3em] text-[#FF5722]">System Overview</h2>
                            <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter text-[#0A1D37]">COMMAND CENTER</h1>
                        </div>
                    ...
                    <div className="grid gap-4 md:gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                        {[
                            ...
                            <div key={s.label} className="relative overflow-hidden rounded-2xl md:rounded-[2rem] border border-gray-100 bg-white p-6 md:p-10 shadow-sm transition-all hover:shadow-xl group">
                                ...
                                <p className="text-[10px] md:text-xs font-black uppercase tracking-widest text-gray-400">{s.label}</p>
                                <p className="mt-2 md:mt-4 text-4xl md:text-6xl font-black italic tracking-tighter text-[#0A1D37]">{s.value}</p>
                            </div>
                        ))}
                    </div>

                    <div className="grid gap-6 md:gap-10 grid-cols-1 lg:grid-cols-2">
                         <section className="rounded-2xl md:rounded-[2.5rem] bg-[#0A1D37] p-6 md:p-10 text-white shadow-2xl flex flex-col justify-between">
                            <div>
                                <h3 className="mb-4 md:mb-8 text-xs font-black uppercase tracking-[0.2em] text-[#FF5722]">User Influx</h3>
                         ...
                         <section className="rounded-2xl md:rounded-[2.5rem] border border-gray-100 bg-white p-6 md:p-10 shadow-sm">
                ...
                <div className="space-y-8 md:space-y-12">
                    ...
                    {myActivities.length > 0 && (
                        (() => {
                            ...
                            return (
                                <div className="mb-4 md:mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 rounded-2xl md:rounded-3xl bg-[#FF5722] text-white px-5 py-4 md:px-8 md:py-6 shadow-xl border-l-4 md:border-l-8 border-[#0A1D37] animate-pulse-slow">
                                    <div className="text-3xl md:text-5xl lg:text-6xl font-black italic mr-0 sm:mr-4">🏁</div>
                                    <div className="flex-1">
                                        <div className="text-[10px] md:text-xs font-black uppercase tracking-widest text-orange-100 mb-1">Incoming Race Reminder</div>
                                        <div className="text-base md:text-2xl font-black italic tracking-tight">{nextRace.event.title}</div>
                                        <div className="mt-1 text-xs md:text-sm font-bold flex flex-wrap gap-2 md:gap-4 items-center">
                                    ...
                                    <Button as={Link} href={route('events.show', nextRace.event_id)} size="sm" variant="secondary" className="mt-2 sm:mt-0 sm:ml-4 shrink-0">View Details</Button>
                                </div>
                            );
                        })()
                    )}

                    <div className="relative overflow-hidden rounded-2xl md:rounded-[3rem] bg-gradient-to-r from-[#0A1D37] to-[#1a3a63] p-6 md:p-12 text-white shadow-2xl">
                        <div className="relative z-10 md:w-2/3">
                            <h2 className="text-xs md:text-sm font-black uppercase tracking-[0.3em] text-[#FF5722]">Welcome Back, Runner</h2>
                            <h1 className="mt-2 text-3xl md:text-5xl lg:text-6xl font-black italic tracking-tighter">READY FOR THE NEXT CHALLENGE?</h1>
                            <p className="mt-4 md:mt-6 text-sm md:text-lg text-gray-300 leading-relaxed max-w-xl">You've got races coming up. Stay focused, stay hydrated, and keep pushing your limits.</p>
                        ...
                        <div className="absolute right-[-10%] top-[-20%] text-[10rem] md:text-[20rem] font-black italic opacity-5 pointer-events-none select-none">RUN</div>
                    </div>

                    <div>
                        <div className="mb-4 md:mb-8 flex items-end justify-between">
                            <h3 className="text-lg md:text-2xl font-black italic tracking-tight text-[#0A1D37]">MY ACTIVE RACES</h3>
                        ...
                        {myActivities.length > 0 ? (
                            <div className="grid gap-4 md:gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                                {calculateRankings(myActivities).map((reg) => (
                                    <div key={reg.id} className="group overflow-hidden rounded-2xl md:rounded-[2.5rem] border border-gray-100 bg-white shadow-md transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                                        {/* Top Banner Area */}
                                        <div className="relative h-32 md:h-40 bg-gray-100 overflow-hidden">
                                            {reg.event.banner_url ? (
                                                ...
                                                <div className="h-full w-full bg-gradient-to-br from-[#0A1D37] to-[#1f3b5e] flex items-center justify-center text-white text-3xl md:text-5xl font-black italic">
                                                    RUN
                                                </div>
                                            )}
                                        ...
                                        <div className="p-5 md:p-8 space-y-4 md:space-y-6">
                                            <div>
                                                <h4 className="text-lg md:text-2xl font-black italic tracking-tighter text-[#0A1D37] leading-none mb-1.5 md:mb-2 group-hover:text-[#FF5722] transition-colors">{reg.event.title}</h4>
                                            ...
                                            <div className="grid grid-cols-2 gap-3 md:gap-4 py-3 md:py-4 border-y border-gray-50 text-left">
                                                <div>
                            ...
                            <div className="rounded-2xl md:rounded-[3rem] border-4 border-dashed border-gray-100 p-10 md:p-20 text-center">
            ...
            {isOrganizer && (
                <div className="space-y-8 md:space-y-12">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
                        <div>
                            <h2 className="text-xs md:text-sm font-black uppercase tracking-[0.3em] text-[#FF5722]">Event Director</h2>
                            <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter text-[#0A1D37]">ORGANIZER HUB</h1>
                        </div>
                        <Button
                            as={Link}
                    ...
                    <div className="grid gap-6 md:gap-8 grid-cols-1 md:grid-cols-2">
                        <section className="rounded-2xl md:rounded-[3rem] border border-gray-100 bg-white p-5 md:p-10 shadow-sm">
                            <div className="mb-6 md:mb-10 flex items-center justify-between">
                                <h3 className="text-base md:text-xl font-black italic uppercase tracking-wider text-[#0A1D37]">Active Events</h3>
                                <span className="rounded-full bg-gray-100 px-3 md:px-4 py-1 text-[10px] font-black text-gray-400">{eventsCount} TOTAL</span>
                            </div>
                            ...
                            {eventsData.length > 0 ? (
                                <div className="space-y-4 md:space-y-6">
                                    {eventsData.map((e) => {
                                        ...
                                        return (
                                            <div key={e.id} className="group rounded-xl md:rounded-[2rem] border border-gray-100 bg-gray-50/30 p-4 md:p-6 lg:p-8 transition-all duration-300 hover:bg-white hover:shadow-xl space-y-4 md:space-y-6">
                                                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                                    <div className="flex items-center gap-4 flex-1 min-w-0">
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            <h4 className="text-sm md:text-lg font-black italic text-[#0A1D37] leading-tight group-hover:text-[#FF5722] transition-colors truncate">{e.title}</h4>
                                                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1 flex items-center gap-1.5 truncate">
                                                                <span className="shrink-0">📅 {new Date(e.date).toLocaleDateString()}</span>
                                                    ...
                                                    <div className="flex items-center gap-2.5 self-start lg:self-center shrink-0 flex-wrap">
                                                        ...
                                                        <Link href={route('events.show', e.id)} className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-[10px] font-black uppercase tracking-widest text-[#0A1D37] hover:border-[#FF5722] hover:text-[#FF5722] transition-all shrink-0">Manage</Link>
                                                    </div>
                                                </div>

                                                {/* Event specifics grid */}
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4 py-3 md:py-4 border-t border-gray-100/70 text-left text-xs">
                                                    <div>
                        ...
                        <section className="space-y-8">
                            <div className="rounded-2xl md:rounded-[3rem] bg-[#FF5722] p-6 md:p-10 text-white shadow-xl shadow-orange-500/20">
                                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-orange-200">Management Tips</h3>
                                <p className="mt-3 md:mt-4 text-xl md:text-2xl font-black italic tracking-tight">ENGAGE YOUR COMMUNITY</p>
                                ...
                                <Button as={Link} href={route('guide')} variant="ghost" size="sm" className="mt-5 md:mt-8 bg-white/20 text-white hover:bg-white/30 backdrop-blur-sm">
                                    Read Guide
                                </Button>
                            </div>
                        </section>
